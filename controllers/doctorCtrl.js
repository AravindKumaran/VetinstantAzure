...
exports.getOtherDoctorDetailFromSameHospital = async (req, res, next) => {
  ...
  const onlineDoc = doctors.find(
    (doc) => doc.user._id !== doctorId && doc.user.isOnline
  );
...
exports.updateDoctorDetail = async (req, res, next) => {
  ...
  const updatedDetails = await Doctor.findByIdAndUpdate(doctorId, req.body, {
    new: true,
  });
  res.status(200).json({
    status: "success",
